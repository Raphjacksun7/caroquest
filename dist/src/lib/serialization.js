...
import { assignPlayerColors, PAWNS_PER_PLAYER, BOARD_SIZE, createInitialGameState, updateBlockingStatus, updateDeadZones, checkWinCondition, } from "./gameLogic";
...
export function serializeGameState(gameState) {
    ...
    const validCurrentPlayerId = gameState.currentPlayerId === 1 || gameState.currentPlayerId === 2
    ...
    const squareOffsets = gameState.board.map((square) => {
        ...
        if (square.pawn) {
            ...
            FbPawn.addColor(builder, square.pawn.color === "light" ? FbSquareColor.Light : FbSquareColor.Dark);
        ...
        FbSquare.addBoardColor(builder, square.boardColor === "light" ? FbSquareColor.Light : FbSquareColor.Dark);
        if (square.pawn) {
    ...
    const boardVectorOffset = FbGameState.createBoardVector(builder, squareOffsets);
    ...
    const gamePhaseValue = gameState.gamePhase === "placement"
    ...
    FbGameState.addSelectedPawnIndex(builder, gameState.selectedPawnIndex !== null ? gameState.selectedPawnIndex : -1);
    FbGameState.addWinner(builder, gameState.winner !== null ? gameState.winner : 0);
    if (gameState.lastMove) {
        FbGameState.addLastMoveFrom(builder, gameState.lastMove.from !== null ? gameState.lastMove.from : -1);
...
export function deserializeGameState(data) {
    try {
        ...
        const currentPlayerId = rawCurrentPlayerId === 1 || rawCurrentPlayerId === 2
            ? rawCurrentPlayerId
            : 1;
        const rawGamePhase = fbState.gamePhase();
        const gamePhase = rawGamePhase === FbGamePhase.Placement ? "placement" : "movement";
        const board = [];
        const boardLength = fbState.boardLength();
        if (boardLength !== BOARD_SIZE * BOARD_SIZE) {
            console.error(`DESERIALIZATION: Expected ${BOARD_SIZE * BOARD_SIZE} squares, got ${boardLength}. Data likely corrupted.`);
        ...
        for (let i = 0; i < boardLength; i++) {
            ...
            if (fbP) {
                ...
                const validPawnPlayerId = pawnPlayerIdRaw === 1 || pawnPlayerIdRaw === 2
                    ? pawnPlayerIdRaw
                    : 1;
                pawn = {
            ...
            board.push({
                ...
                row: fbSq.row(),
                col: fbSq.col(),
                boardColor: fbSq.boardColor() === FbSquareColor.Light ? "light" : "dark",
                pawn,
                highlight,
            });
        }
        const winnerRaw = fbState.winner();
        const winner = winnerRaw === 1 || winnerRaw === 2 ? winnerRaw : null;
        const lastMoveTo = fbState.lastMoveTo();
        const lastMoveFrom = fbState.lastMoveFrom();
        const lastMove = lastMoveTo !== -1
        ...
        const deserialized = {
            ...
            selectedPawnIndex: fbState.selectedPawnIndex() === -1 ? null : fbState.selectedPawnIndex(),
            winner,
            lastMove,
            winningLine: null,
        ...
        const { blockedPawns, blockingPawns } = updateBlockingStatus(deserialized.board);
        deserialized.blockedPawnsInfo = blockedPawns;
        deserialized.blockingPawnsInfo = blockingPawns;
        const { deadZones, deadZoneCreatorPawns } = updateDeadZones(deserialized.board, deserialized.playerColors);
...
export function createDeltaUpdate(previousState, currentState) {
    ...
    for (let i = 0; i < currentState.board.length; i++) {
        ...
        const pawnChanged = JSON.stringify(prevSq.pawn) !== JSON.stringify(currSq.pawn);
    ...
    if (previousState.winner !== currentState.winner ||
    ...
    if (JSON.stringify(previousState.pawnsToPlace) !==
    ...
    if (JSON.stringify(previousState.placedPawns) !==
