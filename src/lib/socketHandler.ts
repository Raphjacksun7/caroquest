...
function sanitizeGameState(state: GameState): GameState {
  const currentPlayerId =
    state.currentPlayerId === 1 || state.currentPlayerId === 2
  ...
  return {
    ...state,
    currentPlayerId,
    playerColors: state.playerColors || { 1: "light", 2: "dark" },
    ...
    selectedPawnIndex:
      state.selectedPawnIndex !== undefined ? state.selectedPawnIndex : null,
    ...
    options: { ...defaultOptions, ...state.options }, // Ensure options are always present
    highlightedValidMoves: state.highlightedValidMoves || [], // Ensure it's an array
  };
}
...
export function setupGameSockets(io: SocketIOServer, gameStore: GameStore) {
    ...
    try {
      ...
      if (
        ...
        if (rateLimitInfo.count > RATE_LIMIT_CONFIG.maxRequests) {
          console.warn(
            `SERVER: Rate limit exceeded for IP: ${clientIp}, Socket ID: ${socket.id}`
          );
  ...
  io.on("connection", (socket: Socket) => {
    ...
    socket.on("join_matchmaking", ({ playerName, rating }) => {
      console.log(
        `SERVER: Matchmaking join request from ${playerName} (Socket: ${socket.id})`
      );
    ...
    socket.on("leave_matchmaking", () => {
      console.log(
        `SERVER: Matchmaking leave request from Socket: ${socket.id}`
      );
    ...
    socket.on("create_game", async ({ playerName, options = {} }) => {
      console.log(
        `SERVER: Create game request from ${playerName} (Socket: ${
          socket.id
      ...
      try {
        ...
        gamePreviousStates.set(
          gameId,
          JSON.parse(JSON.stringify(sanitizedState))
        ); // Store for delta calculation

        const creatorPlayer = game.players.find((p) => p.isCreator);
        console.log(
          `SERVER: Game ${gameId} created by ${creatorPlayer?.name} (P${creatorPlayer?.playerId}). Current turn: P${sanitizedState.currentPlayerId}. Emitting 'game_created'.`
        );
    ...
    socket.on("join_game", async ({ gameId, playerName, options }) => {
      console.log(
        `SERVER: Join game request from ${playerName} (Socket: ${socket.id}) for game ${gameId}`
      );
      try {
        ...
        const result = await gameStore.addPlayerToGame(
          gameId,
          socket.id,
          playerName.trim()
        );
        if (
          ...
          socket.emit("game_error", {
            ...
            errorType: result.error?.includes("full")
              ? "GAME_FULL"
              : "JOIN_FAILED",
            gameId,
          });
        ...
        const connectedCount = updatedGame.players.filter(
          (p) => p.isConnected
        ).length;

        console.log(
          `SERVER: Player ${playerName} (P${result.assignedPlayerId}) joined ${gameId}. Turn: P${sanitizedState.currentPlayerId}. Emitting 'game_joined'.`
        );
        const binaryState = serializeGameState(sanitizedState);
        ...
        socket.emit("game_joined", {
          ...
          opponentName: updatedGame.players.find((p) => p.id !== socket.id)
            ?.name,
        ...
        if (connectedCount === 2) {
          console.log(
            `SERVER: Both players connected in ${gameId}. Turn P${sanitizedState.currentPlayerId}. Broadcasting 'opponent_joined' and 'game_start'.`
          );
        ...
        console.error(
          `SERVER: Error joining game ${gameId}:`,
          error.message,
          error.stack
        );
        socket.emit("game_error", {
    ...
    const handlePlayerAction = async (
      ...
      gameId: string,
      actionData: { squareIndex?: number; fromIndex?: number; toIndex?: number } // clientTimestamp removed as it's not used server-side for validation
    ) => {
      console.log(
        `SERVER: Action '${actionType}' for game ${gameId} from socket ${
          socket.id
      ...
      try {
        ...
        console.log(
          `SERVER: Processing action for ${player.name} (P${player.playerId}). Game turn P${currentState.currentPlayerId}. Phase: ${currentState.gamePhase}`
        );

        if (player.playerId !== currentState.currentPlayerId) {
          console.warn(
            `SERVER: TURN VALIDATION FAILED for ${actionType}. Player ${player.name} (P${player.playerId}) | Game Turn P${currentState.currentPlayerId}.`
          );
          socket.emit("game_error", {
        ...
        console.log(
          `SERVER: Calling game logic for ${actionType}. Player P${player.playerId} acting. State turn P${currentState.currentPlayerId}`
        );

        if (
          actionType === "place_pawn" &&
          typeof actionData.squareIndex === "number"
        ) {
          newState = placePawnLogic(
            currentState,
            actionData.squareIndex,
            player.playerId
          ); // Pass actingPlayerId
        } else if (
        ...
        if (!newState) {
          console.warn(
            `SERVER: GAME LOGIC VALIDATION FAILED. Action '${actionType}' by P${player.playerId} was invalid (gameLogic returned null).`
          );
          socket.emit("game_error", {
        ...
        console.log(
          `SERVER: Action ${actionType} by P${player.playerId} successful. New state turn: P${sanitizedNewState.currentPlayerId}. Phase: ${sanitizedNewState.gamePhase}`
        );

        await gameStore.updateGameState(gameId, sanitizedNewState);

        const previousStateForDelta =
          gamePreviousStates.get(gameId) || currentState;
        const deltaUpdates = createDeltaUpdate(
          previousStateForDelta,
          sanitizedNewState
        );
        gamePreviousStates.set(
          gameId,
          JSON.parse(JSON.stringify(sanitizedNewState))
        );

        const updatedGameData = await gameStore.getGame(gameId);
        if (!updatedGameData) {
          console.error(
            `SERVER: Failed to get game data for ${gameId} after update.`
          );
          return;
        }
        ...
        const shouldSendFullState =
          updatedGameData.sequenceId % 10 === 0 ||
          deltaUpdates.length > 10 ||
          actionType === "place_pawn"; // Send full state more often during placement

        if (shouldSendFullState) {
          console.log(
            `SERVER: Sending full game_updated for ${gameId}. SeqId: ${updatedGameData.sequenceId}. Turn: P${sanitizedNewState.currentPlayerId}`
          );
          ...
          console.log(
            `SERVER: Sending game_delta for ${gameId}. SeqId: ${updatedGameData.sequenceId}, ${deltaUpdates.length} changes. Turn: P${sanitizedNewState.currentPlayerId}`
          );
        ...
        console.error(
          `SERVER: Error handling ${actionType} for game ${gameId}:`,
          error.message,
          error.stack
        );
        socket.emit("game_error", {
    ...
    socket.on("place_pawn", (data) =>
      handlePlayerAction("place_pawn", data.gameId, data)
    );
    socket.on("move_pawn", (data) =>
      handlePlayerAction("move_pawn", data.gameId, data)
    );

    socket.on("request_full_state", async ({ gameId }) => {
      console.log(
        `SERVER: Full state request for game ${gameId} from socket ${socket.id}`
      );
      try {
        const game = await gameStore.getGame(gameId);
        if (game) {
          const sanitizedState = sanitizeGameState(game.state);
          console.log(
            `SERVER: Sending requested full state for ${gameId}. Turn: P${sanitizedState.currentPlayerId}`
          );
          const binaryState = serializeGameState(sanitizedState);
          const compressedState = compress(binaryState);
        ...
        console.error(
          `SERVER: Error sending full state for ${gameId}:`,
          error.message,
          error.stack
        );
        socket.emit("game_error", {
    ...
    socket.on("disconnect", async (reason) => {
      console.log(
        `SERVER: Player disconnected: ${socket.id}, Reason: ${reason}`
      );
      removeFromMatchmakingQueue(socket.id);
      ...
      if (currentJoinedGameId) {
        ...
        try {
          const removedPlayer = await gameStore.removePlayerFromGame(
            gameIdBeforeAsync,
            socket.id
          );
          if (removedPlayer) {
            console.log(
              `SERVER: Player ${removedPlayer.name} (P${removedPlayer.playerId}) marked as disconnected from game ${gameIdBeforeAsync}`
            );
            ...
            if (game) {
              ...
              console.log(
                `SERVER: Game ${gameIdBeforeAsync} no longer exists after player disconnect (possibly cleaned up).`
              );
              gamePreviousStates.delete(gameIdBeforeAsync);
            }
          }
          ...
          console.error(
            `SERVER: Error handling disconnection for game ${gameIdBeforeAsync}:`,
            error.message,
            error.stack
          );
        }
