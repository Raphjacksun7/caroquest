...
import type { GameState, StoredPlayer } from "@/lib/types";
import {
...
import { useGameConnection, useGameStore } from "@/hooks/useGameConnection";
...
export function StrategicPawnsGame() {
  ...
  useEffect(() => {
    const gameIdFromPath = Array.isArray(pathParams?.gameId)
      ? pathParams.gameId[0]
      : pathParams?.gameId;
    if (
      gameIdFromPath &&
      gameIdFromPath !== "local" &&
      gameIdFromPath !== "ai" &&
      gameMode === "select"
    ) {
      setGameMode("remote");
      ...
      if (!isConnected && !isConnecting) {
        connectSocketIO()
          ...
          .catch((err) => {
            console.error("Socket connection failed on mount:", err);
            toast({
              title: t("errorTitle"),
  ...
  }, [gameMode, localGameState, calculateBestMove, isAILoading, aiDifficulty]); // Removed aiError from deps as it's handled in another useEffect
  ...
  useEffect(() => {
    if (activeGameState?.winner) {
      ...
      toast({
        title: t("playerDynamicWins", { playerName: winnerInfo }),
  ...
  const resetGameHandler = useCallback(() => {
    if (gameMode === "remote") {
      // For remote games, reset might need server coordination or be disallowed client-side
      toast({
        title: t("gameReset"),
        description: t("featureNotAvailableRemote"),
      });
    } else {
      // For local or AI games
      setLocalGameState(
        createInitialGameState({ pawnsPerPlayer: PAWNS_PER_PLAYER })
      );
  ...
  const handlePawnDragStart = useCallback(
    (pawnIndex: number) => {
      ...
      if (gameMode === "remote" && remoteSocketGameState)
  ...
  const handleStartGameMode = async (mode: GameMode) => {
    ...
    if (mode === "remote") {
      ...
      try {
        ...
        console.error("Socket connection or game setup failed:", err);
        toast({
          title: t("errorTitle"),
          description: t("connectionFailed"),
          variant: "destructive",
  ...
  useEffect(() => {
    if (gameMode === "remote" && connectedGameId && !pathParams?.gameId) {
      router.push(
        `/game/${connectedGameId}?playerName=${encodeURIComponent(
          ...
          <CardContent className="p-6 sm:p-8 space-y-6">
            <Tabs defaultValue="local" className="w-full">
              ...
              <TabsContent value="local" className="pt-6 space-y-4">
                ...
                <Button
                  onClick={() => handleStartGameMode("local")}
              ...
              <TabsContent value="ai" className="pt-6 space-y-4">
                ...
                <Button
                  onClick={() => handleStartGameMode("ai")}
                  className="w-full bg-primary hover:bg-primary/90 text-primary-foreground py-3 text-base font-semibold rounded-lg shadow-md transition-transform hover:scale-105"
                >
              ...
              <TabsContent value="remote" className="pt-6 space-y-4">
                ...
                <Button
                  onClick={() => handleStartGameMode("remote")}
                  className="w-full bg-primary hover:bg-primary/90 text-primary-foreground py-3 text-base font-semibold rounded-lg shadow-md transition-transform hover:scale-105"
  ...
  if (gameMode === "remote" && isWaitingForOpponent && connectedGameId) {
    return (
      <WaitingRoom
        gameId={connectedGameId}
        playerName={remotePlayerNameInput || ""}
      />
    );
  }
  if (!activeGameState) {
